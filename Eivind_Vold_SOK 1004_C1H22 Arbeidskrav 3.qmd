...
df %>%
  mutate(pertusen = Verdi/1000) %>%
  filter(Tid >= 2000) %>%
  ggplot(aes(x=Tid, y=pertusen, col=var)) +
  geom_line()
...
Ved løpende priser kan vi se at BNP-en har vokst kraftig de siste årene, mens ved faste priser kan vi se en svak økning, etterfulgt av en svak redusering, som stabiliserer seg og øker mot slutten. Ved løpende priser måler vi ved de faktiske prisene det gjeldende året, mens ved faste priser tas det høyde for inflasjon og vi kan dermed se på produksjonutviklingen. vi vil altså se på løpende priser når vi vil se på prisene på de forskjellige årene og utviklingen av dem, mens vi bruker faste priser når vi vil se på produksjonsutviklingen.
...
Df gir oss de 2 observasjonene i samme kolonne, mends df_wide gir dem hver sin kolonne.
...
```{r}
df_wide <- df_wide %>%
  mutate(LBNP = lag(Bruttonasjonalprodukt,n=1L)) %>%
  mutate(LMEMOBNP = lag(`MEMO: Bruttonasjonalprodukt. Faste 2015-priser`,n=1L))

# legger variablene i rekkefølge

df_wide <- df_wide %>%
  relocate("LBNP", .before = `MEMO: Bruttonasjonalprodukt. Faste 2015-priser`)
...
df_wide <- df_wide %>%
  mutate(DBNP = ((`Bruttonasjonalprodukt` - LBNP)/LBNP)*100)%>%
  mutate(DMEMOBNP = (((`MEMO: Bruttonasjonalprodukt. Faste 2015-priser`-LMEMOBNP)/LMEMOBNP))*100)


df_wide <-df_wide %>%
  relocate ("DMEMOBNP", .after = "LMEMOBNP") %>%
  relocate ("DBNP", .after = "LBNP")
...
df_wide <- df_wide %>% 
  relocate(MEMOBNP, .before = "LBNP")
df_long <- df_wide %>% 
  pivot_longer(2:3,names_to = "var", values_to ="verdi")
```
...
Man ser at det er flere observasjoner med negativ vekst i reell bnp, dette skyldes nedgang i produksjonen i disse periodene. Grunner som f.eks finanskrisen eller covid kan være grunner til dette.
