...
Ved å se på ligningen kan vi se at tallet representerer vekstbidraget.
...
df <- df %>%
filter(dato > as.Date("2011-12-31"))
...
df_vekter <- df_ny[-c(0:141),] 

df_vekter <- df_vekter %>%
  mutate(total_row = rowSums(.[3:95])/1000) %>%
  relocate(total_row, .after=dato)
```
...
Konsumprisindeksen måles ved inflasjon. Høye kraftpriser kan indirekte bidra til konsumprisvekst. Høye kraftpriser fører til at strømpriser/strømkostnadene går opp. Dette vil igjen føre til at det blir dyrere å produsere varer og tjenester, som deretter fører til at bedrifter øker prisene som påvirker husholdningen.
